...
class IconDownloader:
    def __init__(self):
        self.output_dir = "downloaded_icons"
        if not os.path.exists(self.output_dir):
            os.makedirs(self.output_dir)
            
        # 使用单一的映射文件和目录
        self.mappings_file = "domain_mappings.json"
        self.mappings_dir = "domain_mappings"
        
        # 加载域名映射
        self.brand_domains = {}
        self.load_all_domain_mappings()
    ...
    def load_all_domain_mappings(self):
        """加载所有域名映射文件"""
        try:
            # 首先加载主映射文件（如果存在）
            if os.path.exists(self.mappings_file):
                with open(self.mappings_file, 'r', encoding='utf-8') as f:
                    self.brand_domains.update(json.load(f))
                    print(f"已加载主映射文件中的 {len(self.brand_domains)} 个域名映射")
            
            # 然后加载domain_mappings目录下的所有JSON文件
                    ...
                    if filename.endswith('.json'):
                        ...
                        try:
                            with open(filepath, 'r', encoding='utf-8') as f:
                                ...
                                self.brand_domains.update(category_mappings)
                                print(f"已加载 {filename} 中的 {len(category_mappings)} 个域名映射")
                        except Exception as e:
                            print(f"加载 {filename} 时出错: {str(e)}")
            
            print(f"总共加载了 {len(self.brand_domains)} 个域名映射")
            
            # 如果没有任何映射，创建默认映射
            if not self.brand_domains:
                print("未找到任何映射，创建默认映射")
                self.create_default_mappings()
                self.load_all_domain_mappings()
                
        except Exception as e:
            print(f"加载域名映射时出错: {str(e)}")
            self.brand_domains = {}
            self.create_default_mappings()
            self.load_all_domain_mappings()
